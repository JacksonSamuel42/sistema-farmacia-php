<?php
	...
	if (empty($_POST['nombre_empresa'])) {
           $errors[] = "Nome da empresa esta vazia";
        }else if (empty($_POST['telefono'])) {
           $errors[] = "Telefone esta vazio";
        } else if (empty($_POST['email'])) {
           $errors[] = "E-mail esta vazio";
        } else if (empty($_POST['moneda'])) {
           $errors[] = "Moeda esta vazio";
        } else if (empty($_POST['direccion'])) {
           $errors[] = "Dirección esta vazio";
		...
		$query_update = mysqli_query($con,$sql);
			if ($query_update){
				$messages[] = "Dados atualizado com sucesso.";
			} else{
				$errors []= "Erro ao atualizar dados por favos tente mais tarde.".mysqli_error($con);
			}
		} else {
